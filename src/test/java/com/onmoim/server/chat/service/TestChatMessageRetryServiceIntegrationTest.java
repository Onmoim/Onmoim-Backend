...
public class TestChatMessageRetryServiceIntegrationTest {
	...
	void testRetryAndRecover() throws Exception {
		...
		doThrow(new RuntimeException("테스트 예외"))
			.when(messagingTemplate).convertAndSend(eq(testDestination), any(RoomChatMessageDto.class));
		...
		verify(messagingTemplate, times(3)).convertAndSend(eq(testDestination), any(RoomChatMessageDto.class));
